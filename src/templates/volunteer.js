...
const Volunteer = ({ dispatch, pageContext, data }) => {
  const { language } = pageContext

  useEffect(() => {
    dispatch(setLocationId({ id: "volunteer", staticPage: true }))
    ...
    <PageWrapper
      title={
        nav.internal
          ...
          .options.filter((i) => i.id === "volunteer")[0].label[language]
      }
      subtitle={
...
export const Head = ({ pageContext }) => (
  <HeadComponent
    ...
    title={
      nav.internal
        .filter((i) => i.id === "how-to")[0]
        .options.filter((i) => i.id === "volunteer")[0].label[
